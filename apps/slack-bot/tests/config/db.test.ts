...
describe('shutdown', () => {
  it('disconnects the Prisma client', async () => {
    const { getPrismaClient, shutdown } = await import('../../src/db/client.js')
  ...
  it('is idempotent — safe to call when no client exists', async () => {
    const { shutdown } = await import('../../src/db/client.js')
    await shutdown()
...
describe('checkConnectivity', () => {
  it('reads schema_versions table from local PostgreSQL', async () => {
    const { checkConnectivity, getPrismaClient, shutdown } = await import('../../src/db/client.js')
    try {
